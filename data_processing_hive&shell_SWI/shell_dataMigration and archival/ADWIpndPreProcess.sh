...
export preprocessscript=ADWIpndPreProcessingOdate.sh
# Assign Exit Codes
...
echo "===================================================================================================" >&2
  ...
  if [[ -n $JOB_NM ]]
    then
      echo "Job Name Passed to the Script: $JOB_NM" >&2
    else
      echo "FAILURE: Job Name Not Passed to Script" >&2
      exit $EXIT_FAILURE
    fi

  if [[ -n $ODATE ]]
    then
      echo "Run Date (Odate) Passed to the Script: $ODATE" >&2
    else
      echo "FAILURE: Run Date (Odate) Not Passed to Script" >&2
      exit $EXIT_FAILURE
    fi
...
# Execute .profile to export variables used in this script PRIVATEKEYFILENAME,CSAREMOTEHOSTNAME,CSAREMOTEHOSTUSERNAME
  ...
  if [[ -e $SCRIPTPATH/$preprocessscript ]]
    then
      ...
      echo "FAILURE: Pre-Process Script  does not Exists in the directory" >&2
      exit $EXIT_FAILURE
    fi
